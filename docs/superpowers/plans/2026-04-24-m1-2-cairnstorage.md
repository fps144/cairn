...
**Tech Stack:** Swift 6.3.1 toolchain · **swift-tools-version 6.0**(升自 5.9,支持 per-target `swiftLanguageMode` API)· GRDB **7.10.0**(自身 Swift 6 严格模式)· CairnCore(M1.1 已就绪)· XCTest in-memory DB。
...
| Task | 谁做 | 依赖 |
|---|---|---|
| T1. Package.swift tools-version 升 6.0 + GRDB 7.10.0 依赖 | Claude | — |
...
- `Package.swift` — tools-version 5.9 → 6.0;GRDB 依赖;CairnStorage target 加 GRDB 依赖;新增 CairnStorageTests target
...
| # | 决策 | 选择 | 理由 |
|---|---|---|---|
| 1 | swift-tools-version | **bump 5.9 → 6.0** | GRDB 7.10.0 自身声明 6.1,SwiftPM 允许依赖 tools-version 更高的包,但我们升到 6.0 可启用 `.swiftLanguageMode` API,为 per-target 控制做准备 |
| 2 | CairnCore / CairnStorage 等目标语言模式 | **显式 `.swiftLanguageMode(.v5)`** | tools-version 6.0+ 默认 Swift 6 严格并发,可能与 SwiftTerm(v5 模式)互操作不畅 + 我们 M1.1 代码尚未审计 Sendable 边界。v1 全部目标锁 v5,避免触发未预料的并发错误。M4.1 后再逐目标迁移 Swift 6 |
...
| 4 | Database 访问隔离 | **actor Database** | spec §3.4 说 "GRDB DAO 内置 serial writer 在 DB 内部";我们把 Database 本身也设为 actor,让调用方 `await db.read/write` 显式跨隔离域。per-target Swift 5 模式下 actor 仍可用(async/await 是 5.5+) |
...
- 测试用 **in-memory DB**(`DatabaseQueue()` 无路径参数),每个测试独立实例,不依赖文件系统

---

## T1:Package.swift tools-version 升 6.0 + GRDB 7.10.0 依赖
...
// per-target Swift 5 语言模式:v1 全部目标锁 v5,避免触发 Swift 6 严格并发
// 未预料错误。M4.1 之后按 target 逐步迁移到 Swift 6。
...
let package = Package(
    ...
    targets: [
        .target(name: "CairnCore", swiftSettings: v5),
        .target(
            name: "CairnStorage",
            dependencies: [
                "CairnCore",
        ...
        .target(
            name: "CairnClaude",
            dependencies: ["CairnCore", "CairnStorage"],
            swiftSettings: v5
        ),
        .target(
            name: "CairnTerminal",
        ...
        .target(
            name: "CairnServices",
            dependencies: ["CairnCore", "CairnStorage", "CairnClaude"],
            swiftSettings: v5
        ),
        .target(
            name: "CairnUI",
            dependencies: ["CairnServices", "CairnTerminal"],
            swiftSettings: v5
        ),
        .executableTarget(name: "CairnApp", dependencies: ["CairnUI"], swiftSettings: v5),
        .testTarget(name: "CairnCoreTests", dependencies: ["CairnCore"], swiftSettings: v5),
        .testTarget(
            name: "CairnStorageTests",
            dependencies: ["CairnStorage"],
            swiftSettings: v5
        ),
    ]
)
...
**变更点**:
1. `// swift-tools-version:6.0`(升自 5.9)
2. 加 GRDB 依赖:`.package(url: "https://github.com/groue/GRDB.swift", from: "7.10.0")`
3. CairnStorage target 加 GRDB product 依赖
4. 每个 target 加 `swiftSettings: v5`(per-target Swift 5 模式)
5. 新增 CairnStorageTests testTarget
...
**失败排查**:
- "package requires swift-tools-version 6.1":若真发生,升 tools-version 到 6.1(GRDB 最严要求)
...
git commit -m "build: tools-version 6.0 + GRDB 7.10.0 依赖 + per-target Swift 5 模式

升 tools-version 5.9 → 6.0 启用 .swiftLanguageMode API。
GRDB 依赖供 CairnStorage 用;其他 target 不碰。
所有 target 显式声明 Swift 5 语言模式(v1 保守策略,M4.1 起按需升)。
...
**注**:上面 `UUID.fetchAll(db, sql:...)` 依赖 GRDB 对 UUID 作为 `DatabaseValueConvertible` 的支持;GRDB 7 默认把 UUID 按 `.uuidString` 编码解码。若此假设错,测试会红,用 `String.fetchAll` + `UUID(uuidString:)` 替代。
...
**Summary**:
- Package.swift tools-version 升 6.0,接入 GRDB 7.10.0;per-target `.swiftLanguageMode(.v5)` 保守策略
...
**交付物**:
- `Package.swift` 升 tools-version 6.0 + GRDB 7.10.0 + per-target Swift 5 模式
...
**风险 1(中)**:**swift-tools-version 6.0 + GRDB 7.10 的实际可行性**。
虽然 GRDB 自身声明 6.1,SwiftPM 理论上允许我们的 6.0 manifest 依赖 6.1 的包(各自 manifest 独立解析)。但实际可能遇到:
- `swift package resolve` 报 "package requires tools-version 6.1"
- 应对:T1 Step 2 失败排查已写明,升我们的 tools-version 到 6.1 即可解决

**风险 2(低)**:**GRDB 7 的 `DatabaseValueConvertible` for UUID 假设**。
plan 多处依赖 `UUID.fetchAll(db, sql:)`,假设 GRDB 7 默认把 UUID 按 uuidString 序列化。
若假设错,测试红,用 `String.fetchAll` + `UUID(uuidString:)` 替代。
...
**风险 4(低)**:**per-target Swift 5 模式能否兼容 GRDB 7 的 Swift 6 API**。
GRDB 7 导出的 `Sendable` 类型在 Swift 5 模式下仍可用(ABI 层面 Sendable 是协议 stamp)。
若遇到 API 签名 Swift 5 不认,说明 GRDB 7 用了 Swift 6 专用语法(`each` 泛型参数包等),需要升我们部分 target 到 Swift 6 模式。但这类情况罕见。
